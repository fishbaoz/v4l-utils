...
#define ModeShown   0x80000000
		...
		for (output = all_outputs; output; output = output->next)
		{
			...
			printf ("%s %s bao", output_info->name, connection[output_info->connection]);
			if (output->primary) {
				printf(" primary");
			}
			printf("%d\n", __LINE__);
			...
			else
			{
				...
				printf("output_info->nmode=%d, output_num=%d\n", output_info->nmode, output_num);
				num_retmodes = 0;
				for (j = 0; j < output_info->nmode; j++)
				{
