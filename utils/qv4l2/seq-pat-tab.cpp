...
bool SeqPatTab::m_fullAudioName = false;
...
void SeqPatTab::sourceChangeSubscribe()
{
	v4l2_input vin;
...
void SeqPatTab::SequenceSection(v4l2_input vin)
{
	bool needsStd = false;
	...
	xrandr.closeWriteChannel();

	//QByteArray xrandrOutput;
	QString xrandrOutput;

	while (false == xrandr.waitForFinished()) {
		qDebug("finish xrandr failed");
		return ;
	}
	xrandrOutput = QString::fromLocal8Bit(xrandr.readAll());
	QRegExp rx("(\\d+x\\d+\\s+)");
	int pos = 0;
	addLabel("Sequence");
	...
	addWidget(m_seqOutput);
	connect(m_seqOutput, SIGNAL(activated(int)), SLOT(seqOutputChanged(int)));
	m_row++;
	m_col = 0;
//	qDebug("%s", qPrintable(xrandrOutput));
//	if (!isRadio() && !enum_input(vin, true)) {
		addLabel("Input");
		m_videoInput = new QComboBox(parentWidget());
		// Get screen recolution
...
unsigned SeqPatTab::getNumBuffers() const
{
	return m_numBuffers ? m_numBuffers->value() : 4;
}

void SeqPatTab::setHaveBuffers(bool haveBuffers)
{
...
bool SeqPatTab::filterAudioDevice(QString &deviceName)
{
	// Only show hw devices
	return deviceName.contains("hw") && !deviceName.contains("plughw");
}

int SeqPatTab::addAudioDevice(void *hint, int deviceNum)
{
	int added = 0;
#ifdef HAVE_ALSA
	char *name;
...
bool SeqPatTab::createAudioDeviceList()
{
#ifdef HAVE_ALSA
...
void SeqPatTab::addTitle(const QString &titlename)
{
	m_row++;
...
void SeqPatTab::updateGUIOutput(__u32 output)
{
	v4l2_output out;
...
void SeqPatTab::inputChanged(int input)
{
	s_input((__u32)input);
...
void SeqPatTab::seqOutputChanged(int input)
{
	s_input((__u32)input);
...
void SeqPatTab::inputAudioChanged(int input)
{
	s_audio((__u32)input);
	updateAudioInput();
}

void SeqPatTab::outputAudioChanged(int output)
{
	s_audout((__u32)output);
	updateAudioOutput();
}

void SeqPatTab::standardChanged(int std)
{
	v4l2_standard vs;
...
void SeqPatTab::timingsChanged(int index)
{
...
void SeqPatTab::freqTableChanged(int)
{
	updateFreqChannel();
	freqChannelChanged(0);
}

void SeqPatTab::freqChannelChanged(int idx)
{
...
void SeqPatTab::freqChanged(double f)
{
	v4l2_frequency freq;
...
void SeqPatTab::freqRfChanged(double f)
{
	v4l2_frequency freq;
...
void SeqPatTab::audioModeChanged(int)
{
	m_tuner.audmode = m_audioModes[m_audioMode->currentIndex()];
	s_tuner(m_tuner);
}

void SeqPatTab::detectSubchansClicked()
{
	QString chans;
...
void SeqPatTab::stereoModeChanged()
{
	v4l2_modulator mod;
...
void SeqPatTab::colorspaceChanged(int idx)
{
	cv4l_fmt fmt;
...
void SeqPatTab::xferFuncChanged(int idx)
{
	cv4l_fmt fmt;
...
void SeqPatTab::ycbcrEncChanged(int idx)
{
	cv4l_fmt fmt;
...
void SeqPatTab::quantRangeChanged(int idx)
{
	cv4l_fmt fmt;
...
void SeqPatTab::clearColorspace(cv4l_fmt &fmt)
{
...
void SeqPatTab::vidCapFormatChanged(int idx)
{
	v4l2_fmtdesc desc;
...
void SeqPatTab::vidFieldChanged(int idx)
{
	cv4l_fmt fmt;
...
void SeqPatTab::frameWidthChanged()
{
	cv4l_fmt fmt;
...
void SeqPatTab::frameHeightChanged()
{
	cv4l_fmt fmt;
...
void SeqPatTab::frameSizeChanged(int idx)
{
...
void SeqPatTab::frameIntervalChanged(int idx)
{
...
void SeqPatTab::vidOutFormatChanged(int idx)
{
	v4l2_fmtdesc desc;
...
void SeqPatTab::vbiMethodsChanged(int idx)
{
	if (isSlicedVbi())
...
void SeqPatTab::updateVideoInput()
{
	__u32 input;
	v4l2_input in;
...
void SeqPatTab::updateVideoOutput()
{
	__u32 output;
	v4l2_output out;
...
void SeqPatTab::updateAudioInput()
{
	v4l2_audio audio;
	QString what;
...
void SeqPatTab::updateAudioOutput()
{
	v4l2_audioout audio;
...
void SeqPatTab::refreshStandards()
{
	v4l2_standard vs;
	m_tvStandard->clear();
	if (!enum_std(vs, true)) {
...
void SeqPatTab::qryTimingsClicked()
{
	v4l2_dv_timings timings;
...
void SeqPatTab::sourceChange(const v4l2_event &ev)
{
...
void SeqPatTab::updateFreqChannel()
{
	m_freqChannel->clear();
...
void SeqPatTab::updateColorspace()
{
	cv4l_fmt fmt;
	int idx;
...
void SeqPatTab::updateVidCapFormat()
{
	v4l2_fmtdesc desc;
	cv4l_fmt fmt;
...
void SeqPatTab::updateVidOutFormat()
{
	v4l2_fmtdesc desc;
	cv4l_fmt fmt;
...
void SeqPatTab::updateVidFields()
{
	cv4l_fmt fmt;
	cv4l_fmt tmp;
	bool first = true;
...
void SeqPatTab::updateFrameSize()
{
...
CropMethod SeqPatTab::getCropMethod()
{
...
void SeqPatTab::changePixelAspectRatio()
{
	// Update hints by calling a get
	getPixelAspectRatio();
	info("");
...
double SeqPatTab::getPixelAspectRatio()
{
...
QString SeqPatTab::getAudioInDevice()
{
...
QString SeqPatTab::getAudioOutDevice()
{
...
void SeqPatTab::setAudioDeviceBufferSize(int size)
{
	m_audioDeviceBufferSize = size;
}

int SeqPatTab::getAudioDeviceBufferSize()
{
	return m_audioDeviceBufferSize;
}

#ifdef HAVE_ALSA
int SeqPatTab::checkMatchAudioDevice(void *md, const char *vid, enum device_type type)
{
...
int SeqPatTab::matchAudioDevice()
{
